##this file downloads records names for given database name and reads rr intervals and uses raw rr intervals to plot histograms.
...
#for record in range(1):
    ...
    output_folder="/home/ubuntu/Documents/Thesis_work/RR_variability/09_oct_features/histograms/normal/normal_bins_constant/"
    ...
    fig.savefig(output_folder+"hist_"+record+".png")
...
#     plt.title("DELTA RR interval for %s" % rec_name);
    ...
    #sdrr_curr_ddof=np.std(RR_sec_detrended_ms, ddof=1);
    print("SDRR after filtering and detrending in ms for " + rec_name + "is: " + str(sdrr_curr));
    #print("sample SDRR in ms for " + rec_name + "is: " + str(sdrr_curr_ddof));
    
    
    ### plot RR intervals array ###
#     plt.figure()
#     plt.plot(range(len(RR_sec)), RR_sec,color='blue');
#     plt.xlabel("beats")
